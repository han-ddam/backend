...
| **응답 포맷(envelope)** | 성공은 **`{ result:<payload> }`** 만, 실패는 **`{ error:{ code, message } }`** 만 (성공엔 error 없음, 실패엔 result 없음). **아래 표의 응답 형태는 `result` 안의 payload**. 204(logout 등)는 본문 없음. |
| **에러 포맷** | `{ error:{ code, message } }`. `code`=HTTP 상태명(`NOT_FOUND`/`BAD_REQUEST`/`UNAUTHORIZED`…). 검증 실패 400/422, 인증 401, 권한 403, 충돌 409. |
