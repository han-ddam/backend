...
collection        id, source(CURATED|USER), owner_user_id?, type(THEME|REGION),
                  region_code?(REGION), theme_tags text[], is_ordered,
                  visibility, like_count, created_at
...
- **테마**: `collection.theme_tags text[]` (배열, tag 느낌) — 컬렉션 단위 속성. place는 여러 컬렉션 소속 가능.
- 도감 탭: **지역별**(region 진행도 집계) / **테마별**(collection.theme_tags) / **최근수집**.
...
## 15. 도감·진행도·큐레이션·인증정책 (확정)

1. **진행도 분모 = scope별 등록(도감 대상) place 수**
   - 전국 도감 = 인증 수 / 전체 등록 place(예 **161**), 시·도 = 인증 수 / 해당 도 place(예 강원 **21**).
   - 분모 = `COUNT(place WHERE region/province = scope)`. 전국 = 전체 place. (등록된 place = 도감 대상)
2. **테마 = `collection.theme_tags text[]`** (배열, tag 느낌). place는 여러 컬렉션 소속 가능.
3. **place 점수값 수동** — `base_points`·`rarity_weight`는 **어드민이 등록 시 수동 설정**(자동 아님).
4. **TourAPI 동기화 = 관광지만** (`contentTypeId=12`). 그 외 타입은 동기화 안 함.
5. **인증 검증 정책 = 별도 테이블** (데이터 기반)
   ```
   cert_policy  id, scope(GLOBAL|REGION|CATEGORY), gps_tolerance_m,
                composition_required bool, composition_min_score numeric,
                effective_from/to
   ```
   - GPS 허용오차·구도매칭 필수여부/임계를 코드 아닌 데이터로 → 정책 변경 = 행 수정.
   - place 큐레이션: TourAPI 관광지 후보 → 어드민이 선별 등록(점수 수동) → 등록된 것이 도감 대상.
